...
export async function PATCH(
  ...
  try {
    ...
    if (isOnline) {
      ...
      departmentId = issue.audit.departmentId;

      // Permission Gate: Admin or Owner of the issue
      const employee = await prisma.employee.findUnique({ where: { email: userEmail } });
      const employeeName = employee ? employee.name : '';

      const isOwner = issue.owner === userEmail || (employeeName && issue.owner.includes(employeeName));
      if (userRole !== 'Admin' && !isOwner) {
        return NextResponse.json({ error: 'Unauthorized. You must be the owner or Admin.' }, { status: 403 });
      }

      updatedIssue = await prisma.complianceIssue.update({
        where: { id },
        data: { status },
        include: {
          audit: true,
      ...
      // Recalculate score (High/Critical open issue count changed)
      await updateScorePipeline(departmentId, isOnline);

    } else {
      const db = getFallbackDb();
      const issueIndex = db.complianceIssues.findIndex((i: any) => i.id === id);

      if (issueIndex < 0) {
        return NextResponse.json({ error: 'Compliance issue not found.' }, { status: 404 });
      }

      const issue = db.complianceIssues[issueIndex];
      const audit = db.audits.find((a: any) => a.id === issue.auditId);
      if (!audit) {
        return NextResponse.json({ error: 'Linked audit not found.' }, { status: 404 });
      }
      ...
      const isOwner = issue.owner === userEmail || (employeeName && issue.owner.toLowerCase().includes(employeeName.toLowerCase()));
      if (userRole !== 'Admin' && !isOwner) {
        return NextResponse.json({ error: 'Unauthorized. You must be the owner or Admin.' }, { status: 403 });
      }

      db.complianceIssues[issueIndex] = {
        ...issue,
        status,
        updatedAt: new Date().toISOString(),
      };
      updatedIssue = db.complianceIssues[issueIndex];
      saveFallbackDb(db);
